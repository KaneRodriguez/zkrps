...
const main = async () => {
  ...
  const minimalGame = await deploy('MinimalGame'); // <-- add in constructor args like line 19 vvvv
  // use for local token bridging
  // const mockToken = await deploy("MockERC20") // <-- add in constructor args like line 19 vvvv

  // const minimalGame = await ethers.getContractAt('MinimalGame', "0xaAC799eC2d00C013f1F11c37E654e59B0429DF6A") //<-- if you want to instantiate a version of a contract at a specific address!
  ...
  */
  ...
  const minimalGame = await deploy("MinimalGame", [], {
  ...
  */
  ...
  await tenderlyVerify(
    {contractName: "MinimalGame",
     contractAddress: minimalGame.address
  })
  */
...
const isSolidity = (fileName) => fileName.indexOf('.sol') >= 0 && fileName.indexOf('.swp') < 0 && fileName.indexOf('.swap') < 0;
